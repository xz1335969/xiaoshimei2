    ...
    if 8 < datetime.datetime.now().hour <= 23:
        ...
        if not len(result):
            ...
            for accounts in result:
                if datetime.datetime.strptime(accounts[1], "%Y-%m-%d %H:%M:%S") <= datetime.datetime.now():
                ...
                目前使用情况：{}
                =========================
                """.format(accounts[3],accounts[0],current_user)
            report += "请选择您要使用的账号ID，输入其他内容取消"
    else:
        report = "目前不在可使用时间范围内"
        await public_account.finish(report)
...
async def _(event:PrivateMessageEvent,bot:Bot,state:T_State):
    ...
    try:
        ...
        if datetime.datetime.strptime(result[3], "%Y-%m-%d %H:%M:%S") > datetime.datetime.now():
            await public_account.finish("此账号40分钟内有人使用，为避免顶号，请选择其他号或联系使用者QQ归还。")
        report = f"您要使用的账号如下：\n账号：{result[0]}\n密码：{result[1]}"
        msg_id = await public_account.send(report)
        sqlquery(2,"public_account_usage",None,("id","user_id","group_id","end_time"),(account_id,user_id,group_id,f"\'{(datetime.datetime.now()+datetime.timedelta(minutes=40)).strftime('%Y-%m-%d %H:%M:%S')}\'"))
        sqlquery(1,"public_account",{"id":account_id},("user_id",user_id),("limit_time",f"\'{(datetime.datetime.now()+datetime.timedelta(minutes=40)).strftime('%Y-%m-%d %H:%M:%S')}\'"))
        time.sleep(35)
...
async def _(event:PrivateMessageEvent):
    user_id = event.user_id
    group_id = event.sender.group_id
    result = sqlquery(0,"public_account",{"user_id":user_id,"group_id":group_id},("limit_time","id"),fetchone=True,others="ORDERED BY usage_id LIMIT 1")
    if datetime.datetime.strptime(result[0],"%Y-%m-%d %H:%M:%S") <= datetime.datetime.now():
        await return_account.finish("您目前没有需要归还的账号，或者距离您上次借用公共号已超过40分钟")
    else:
        account_id = result[1]
        sqlquery(1,"public_account",{"id":account_id},("limit_time",f"\'{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\'"))
        sqlquery(1,"public_account_usage",{"id":account_id},("end_time",f"\'{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\'"))
...
async def _(event:PrivateMessageEvent,state:T_State):
    ...
    group_id = event.sender.group_id
    sqlquery(2,"public_account",None,("account","password","nickname","group_id"),(f"\'{account}\'",f"\'{password}\'",f"\'{nickname}\'",group_id))
